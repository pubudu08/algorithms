...
def quick_sort(aList):
    ...
    """
    return quick_sort_operation(aList, 0, len(aList) - 1)


def quick_sort_operation(aList, first, last):
    if first < last:
        pivot = partition(aList, first, last)
        quick_sort_operation(aList, first, pivot - 1)
        quick_sort_operation(aList, pivot + 1, last)
    return aList
...
def partition(aList, first, last):
    ...
    for i in range(first, last):
        if aList[i] <= aList[last]:
            swap_quick_sort(aList, i, first)
            first += 1
    swap_quick_sort(aList, first, last)
    return first


def swap_quick_sort(A, x, y):
    A[x], A[y] = A[y], A[x]


# am = [8, 5, 3, 1, 9, 6, 0, 7, 4, 2]
# print(quick_sort(am))
...
def heap_sort(aList):
    ...
    more info: https://www.youtube.com/watch?v=D_B3HN4gcUA
    """
    length = len(aList) - 1  # converting list in to heap
    latest_parent = length // 2
    for i in range(latest_parent, -1, -1):
        move_down(aList, i, length)

    # flatten heap in to sorted array
    for i in range(length, 0, -1):
        if aList[0] > aList[i]:
            swap(aList, 0, i)
            move_down(aList, 0, i - 1)
    return aList


def move_down(aList, first, last):
    largest = 2 * first + 1
    while largest <= last:
        # right child exist and it is larger than the left child
        if largest < last and aList[largest] < aList[largest + 1]:
            largest += 1
        # right child is larger than parent
        if aList[largest] > aList[first]:
            swap(aList, largest, first)
            # move down to larger child
            first = largest
            largest = 2 * first + 1
        else:
            return
...
# heap_list = [8, 5, 3, 1, 9, 6, 0, 7, 4, 2, 5]
